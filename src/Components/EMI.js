...
const EMI = () => {
  ...
  const localName=localStorage.getItem("nameData")
    ...
    const calculateEmi = () =>{
        ...
        const r = rate/ (12*100);
        const emi =  loanAmnt * r * (1+ r) ** duration / ((1+r) ** duration - 1);
        setEmi(emi.toFixed(2));
        const newHistory = [...history, {loanAmnt,rate,duration, emi: emi.toFixed(2)}]
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: "30px" }}>
        <Button variant="outlined" onClick={calculateEmi}>Submit</Button>
      </div>
     
      {
      (!getAmount&&!getRate&&!getDuration)? history.length > 0 && (
